...
class StreamCharBufferSpec extends Specification {
    ...
    def "stream char buffer should support automaticly encoding to connected writer"() {
        given:
        def connectedBuffer = new StreamCharBuffer()
        buffer.encodeInStreamingModeTo([getEncoder: { ->
            htmlCodecClass.encoder }] as EncoderAware, EncodingStateRegistryLookupHolder.getEncodingStateRegistryLookup(), true, connectedBuffer.writer)
        when:
        def hello = "Hello world & hi"
        out << hello
        out.flush()
